<template>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchAlbums() {
        try {
          const response = await axios.get('http://localhost:8085/photo/favorite',null,{
      ...
      async fetchAlbumsWithLike() {
        try {
          const response = await axios.post('http://localhost:8085/photo/favorite',null,{
            params: { username : this.currentUser.name }
          }); // 向后端发送请求
          if (response.data.code === 1) {
            this.albums = response.data.data; // 更新相册数据
